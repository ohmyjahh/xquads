...
- **2026-09-21** — Publicada. `/xquads/linkedinauto` responde 200, LeadGate ativo, os 3 prompts protegidos (nenhum `<pre>` no DOM antes do lead), as três etapas e os dois avisos remanescentes visíveis sem preencher formulário. Confirmado em produção que o card sobre a política de automação não está mais na página. Auto-deploy do Vercel disparou pelo push.
